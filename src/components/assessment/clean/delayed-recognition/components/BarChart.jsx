// BarChart
// -----------------------------------------------------------------------------
// Shared, dependency-free inline SVG bar chart. No charting library --
...
export default function BarChart({ labels, values, barColor, maxValue = 100 }) {
  ...
  return (
    <svg viewBox={`0 0 ${width} ${height}`} width="100%" role="img" aria-label="Bar chart">
      <line x1={pad} y1={height - pad} x2={width - pad} y2={height - pad} stroke="var(--drt-border-strong)" />
      {values.map((v, i) => {
        ...
        return (
          <g key={i}>
            ...
            <text x={x + barWidth / 2} y={height - pad + 16} fontSize="12" textAnchor="middle" fill="var(--drt-text-secondary)">
              {labels[i]}
            </text>
          </g>
        );

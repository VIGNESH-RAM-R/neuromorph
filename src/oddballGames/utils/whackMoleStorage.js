import { WHACK_MOLE_STORAGE_KEY } from '../config/whackMoleConfig.js';
import { isBrowserStorageAvailable, getDeviceInfo } from './browserEnv.js';

/**
 * Local persistence for completed Whack the Mole assessments. As with the
 ...
 * without touching assessment/UI logic (spec section 63).
 */
...
export function loadAssessments() {
  ...
  try {
    const raw = window.localStorage.getItem(WHACK_MOLE_STORAGE_KEY);
    if (!raw) return [];
...
export function saveAssessment(assessment) {
  ...
  try {
    ...
    window.localStorage.setItem(WHACK_MOLE_STORAGE_KEY, JSON.stringify(updated));
    return updated;
  } catch {
